...
const loadUserList = async () => {
  ...
  try {
    ...
    const res = await getAmazonCustomerList(params)
    if (res && res.data) {
      // 将数据库数据转换为表格显示格式
...
const handleDelete = async (record) => {
  try {
    if (isExcelMode.value) {
      ...
      await deleteAmazonCustomer(deleteId)
      message.success('删除成功')
    ...
    onOk: async () => {
      try {
        ...
        if (isExcelMode.value) {
          ...
          const res = await deleteAmazonCustomers(idsToDelete)
...
const handleSubmit = async () => {
  try {
    ...
    if (formData.id) {
      await updateAmazonCustomer(formData.id, submitData)
      message.success('更新成功')
    } else {
      await createAmazonCustomer(submitData)
...
const handleImport = async (file) => {
  try {
    ...
    try {
      ...
      try {
        loading.value = true
        const res = await importAmazonCustomers(file)
        if (res && res.data) {
