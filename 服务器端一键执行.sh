...
if [ ! -f "/opt/wladmin/backend/package.json" ]; then
    ...
    echo "   在本地执行: scp -r server/* root@182.92.59.70:/opt/wladmin/backend/"
    exit 1
fi
if [ ! -f "/opt/wladmin/frontend/index.html" ]; then
    echo "⚠️  前端文件不存在，请先上传文件"
    echo "   在本地执行: scp -r dist/* root@182.92.59.70:/opt/wladmin/frontend/"
    exit 1
fi
echo "✅ 文件检查完成"
...
echo ""
echo "1️⃣1️⃣ 测试服务..."
curl -s http://localhost:3002/api/health && echo "" || echo "后端健康检查失败"
curl -s http://localhost:3004 | head -3 || echo "前端访问失败"
...
echo ""
echo "如果无法访问，请检查阿里云安全组是否开放了端口3004和3002"
echo "=========================================="
