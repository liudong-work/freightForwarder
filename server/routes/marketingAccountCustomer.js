...
import {
  ...
  getMarketingAccountCustomerById,
  ...
  deleteAllMarketingAccountCustomers,
  importMarketingAccountCustomers
} from '../services/marketingAccountCustomerService.js'
...
router.get('/', async (req, res) => {
  try {
    ...
    const result = await getMarketingAccountCustomerList(params, req.user)
    res.json({
      code: 200,
      data: result,
      message: '获取成功'
    ...
    console.error('获取营销号客户列表错误:', error)
    res.status(500).json({
      code: 500,
...
router.get('/:id', async (req, res) => {
  try {
    const { id } = req.params
    const customer = await getMarketingAccountCustomerById(id)
...
router.post('/', async (req, res) => {
  try {
    ...
    const customer = await createMarketingAccountCustomer(customerData, req.user.id)
...
router.put('/:id', async (req, res) => {
  try {
    const { id } = req.params
    const customerData = req.body
    const customer = await updateMarketingAccountCustomer(id, customerData)
...
router.delete('/:id', async (req, res) => {
  try {
    const { id } = req.params
    await deleteMarketingAccountCustomer(id)
...
router.delete('/', async (req, res) => {
  try {
    ...
    if (ids === 'all') {
      const deletedCount = await deleteAllMarketingAccountCustomers()
      return res.json({
        code: 200,
        data: { deletedCount },
    ...
    const deletedCount = await deleteMarketingAccountCustomers(ids)
...
router.post('/import', upload.single('file'), async (req, res) => {
  try {
    ...
    // 批量导入客户
    try {
      const results = await importMarketingAccountCustomers(customersData, req.user.id)
