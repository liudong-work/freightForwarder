...
} from '../services/alibabaCustomerService.js'
...
router.get('/', async (req, res) => {
  try {
    ...
    const result = await getAlibabaCustomerList(params, req.user)
    res.json({
      code: 200,
      data: result,
      message: '获取成功'
    ...
    console.error('获取阿里巴巴客户列表错误:', error)
    res.status(500).json({
      code: 500,
...
router.get('/:id', async (req, res) => {
  try {
    const { id } = req.params
    const customer = await getAlibabaCustomerById(id)
...
router.post('/', async (req, res) => {
  try {
    ...
    const customer = await createAlibabaCustomer(customerData, req.user.id)
...
router.put('/:id', async (req, res) => {
  try {
    const { id } = req.params
    const customerData = req.body
    const customer = await updateAlibabaCustomer(id, customerData)
...
router.delete('/:id', async (req, res) => {
  try {
    const { id } = req.params
    await deleteAlibabaCustomer(id)
...
router.delete('/', async (req, res) => {
  try {
    ...
    if (ids === 'all') {
      const deletedCount = await deleteAllAlibabaCustomers()
      return res.json({
        code: 200,
        data: { deletedCount },
    ...
    const deletedCount = await deleteAlibabaCustomers(ids)
...
router.post('/import', upload.single('file'), async (req, res) => {
  try {
    ...
    // 批量导入客户
    try {
      const results = await importAlibabaCustomers(customersData, req.user.id)
