import MarketingAccountCustomer from '../models/MarketingAccountCustomer.js'
...
// 获取营销号客户列表（带分页和筛选）
export async function getMarketingAccountCustomerList(params, currentUser) {
  ...
  try {
    ...
    // 获取总数
    const total = await MarketingAccountCustomer.countDocuments(query)
    
    // 分页查询
    const skip = (page - 1) * pageSize
    const list = await MarketingAccountCustomer.find(query)
      .sort({ createdAt: -1 })
      .skip(skip)
...
// 获取亚马逊客户详情
export async function getMarketingAccountCustomerById(id) {
  try {
    const customer = await MarketingAccountCustomer.findById(id).lean()
    return customer
  } catch (error) {
...
// 创建亚马逊客户
export async function createMarketingAccountCustomer(customerData, createdById) {
  try {
    const customerObj = {
    ...
    const newCustomer = new MarketingAccountCustomer(customerObj)
...
// 更新亚马逊客户
export async function updateMarketingAccountCustomer(id, customerData) {
  try {
    const updateObj = {}
    ...
    const updatedCustomer = await MarketingAccountCustomer.findByIdAndUpdate(
      id,
      { $set: updateObj },
...
// 删除亚马逊客户
export async function deleteMarketingAccountCustomer(id) {
  try {
    // 验证ID格式
    if (!id) {
    ...
    const deletedCustomer = await MarketingAccountCustomer.findByIdAndDelete(objectId)
    if (!deletedCustomer) {
      throw new Error('客户不存在')
    }
    return true
...
// 批量删除亚马逊客户
export async function deleteMarketingAccountCustomers(ids) {
  try {
    ...
    const result = await MarketingAccountCustomer.deleteMany({ _id: { $in: objectIds } })
...
// 删除所有亚马逊客户（测试用）
export async function deleteAllMarketingAccountCustomers() {
  try {
    const result = await MarketingAccountCustomer.deleteMany({})
    return result.deletedCount
  } catch (error) {
...
// 批量导入亚马逊客户
export async function importMarketingAccountCustomers(customersData, createdById) {
  // 检查MongoDB连接状态
  ...
  for (let i = 0; i < customersData.length; i++) {
    ...
    try {
      ...
      const newCustomer = new MarketingAccountCustomer(mappedData)
      const savedCustomer = await newCustomer.save()
